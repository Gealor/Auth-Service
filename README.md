...
При дергании эндпоинта /auth/login, в ответе возвращается два токена: access и refresh. Если вы пользуетесь автодокументацией Swagger, то скопируйте access токен и вставьте его в поле, которое вы найдете здесь.
![alt text](image.png)
Аналогично, если вы захотите обновить токены с помощью эндпоинта /auth/refresh, то скопируйте refresh токен и вставьте туда же.
Если вы пользуетесь Postman или д.р. то при отправке http-запроса добавьте заголовок Authorization и к токену добавьте приставку Bearer.
